...
{
    public class CosmosDbThingyGetMessagesQueryHandler : IQueryHandler<ThingyGetMessagesQuery, IReadOnlyCollection<ThingyMessage>>
    {
        ...
        public async Task<IReadOnlyCollection<ThingyMessage>> ExecuteQueryAsync(ThingyGetMessagesQuery query, CancellationToken cancellationToken)
        {
            ...
            var messages = _readStore.GetItemLinqQueryable().Where(x => x.ThingyId == thingyId).ToList().Select(x => x.ToThingyMessage())
                .ToList();
            return messages;
        }
    }
