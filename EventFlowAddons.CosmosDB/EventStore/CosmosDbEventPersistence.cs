...
public class CosmosDbEventPersistence: IEventPersistence
{
    ...
    public CosmosDbEventPersistence(ILogger<CosmosDbEventPersistence> logger, Database database, ICosmosDbEventSequenceStore cosmosDbEventSequenceStore)
    {
        _logger = logger;
        _database = database;
    ...
    public async Task<AllCommittedEventsPage> LoadAllCommittedEvents(GlobalPosition globalPosition, int pageSize, CancellationToken cancellationToken)
    {
        ...
        var allCommittedEvents = await events.Select(@event => @event).ToListAsync(cancellationToken: cancellationToken);
    ...
    public async Task<IReadOnlyCollection<ICommittedDomainEvent>> CommitEventsAsync(IIdentity id, IReadOnlyCollection<SerializedEvent> serializedEvents, CancellationToken cancellationToken)
    {
        ...
        foreach (var @event in serializedEvents)
        {
            var globalSequenceNumber =
                await _cosmosDbEventSequenceStore.GetLastSequenceNumberAsync(cancellationToken: cancellationToken);
        ...
        {
            batch.CreateItem(committedEvent);
        }
        
        var response = await batch.ExecuteAsync(cancellationToken: cancellationToken);
        if (response.IsSuccessStatusCode) return committedEvents;
        
        _logger.LogError("Failed to commit events to CosmosDB. Response status code: {StatusCode}", response.StatusCode);
    ...
    public async Task<IReadOnlyCollection<ICommittedDomainEvent>> LoadCommittedEventsAsync(IIdentity id, int fromEventSequenceNumber, CancellationToken cancellationToken)
    {
        ...
        var committedEvents = await events.Select(@event => @event).ToListAsync(cancellationToken: cancellationToken);
    ...
    public async Task DeleteEventsAsync(IIdentity id, CancellationToken cancellationToken)
    {
        ...
        {
            await _eventContainer.DeleteItemAsync<CosmosDbEventDataModel>(@event.id, new PartitionKey(@event.AggregateId), cancellationToken: cancellationToken);
        }
    }
